 ...
 *  QuaternionSlerp
 *
 *  Based on QuaternionTransform. This example added on top slerp functionality for quaternion.
 *  Instead of sudden change to update according to the facing direction vector changed from GUI, it
 *  will take time using slerp (via t factor) in order to update to the final destination facing direction.
 *
 *  Notable function for this example:
 *      glm::quat quatSlerp(const glm::quat& start, const glm::quat& end, float t);
 *
 */
#include "imgui.h"
...
#define SLERP_T 0.10
glm::quat plane1Quat = glm::identity<glm::quat>();
...
/// slerp (spherical linear interpolation) for for input quaternion `start` and `end` for `t` amount
/// which is [0,1].
/// thus this expects application code to maintain a quaternion for object, and have a copy of target
/// quaternion to slerp into.
glm::quat quatSlerp(const glm::quat& start, const glm::quat& end, float t)
{
    // make a copy of start quanternion as we might have to negate one of two operands quaternion
    // to satisfy the shortest arc operation (positive dot product)
    glm::quat startCopy = glm::quat(start);

    float cosOmega = glm::dot(startCopy, end);

    // check to use shortest arc, thus dot product needs to be positive
    if (cosOmega < 0.0f)
    {
        // (completely) negate the quaternion (not just conjugate)
        startCopy = -startCopy;
        cosOmega = glm::dot(startCopy, end);
    }

    float k0, k1;

    // check if both quaternions are really close to each other, then use nomral linear interpolation
    // instead to avoid divide by zero from sin(~0) term
    if (cosOmega > 1.0f - 0.001f)
    {
        k0 = 1.0-t;
        k1 = t;
    }
    ...
    {
        // find omega angle from arctan2 to get the truly angle slope in all possible orientation (all quadrants)
        // rather than just arcsin.
        // first we need to find sin(omega) (we already had cos(omega))
        float sinOmega = std::sqrt(1 - cosOmega*cosOmega);
        float omega = std::atan2(sinOmega, cosOmega);

        // compute k0 and k1 constants
        // not use glm::mix here as such function is more or less the complete version of 
        // interpolation included slerp (rather than glm::slerp which includes parallel check)
        k0 = std::sin((1-t)*omega)/sinOmega;
        k1 = std::sin(t*omega)/sinOmega;
    }

    return k0 * startCopy + k1 * end;
}
...
void render()
{
    ...
    glBindVertexArray(sharedVAO);
        glBindBuffer(GL_ARRAY_BUFFER, sharedVBO);
        // plane
        // slerp quaternion to the target with configured factor t
        plane1Quat = quatSlerp(plane1Quat, targetFacingQuat, SLERP_T);
        // convert quaternion to matrix for drawing
        glm::mat3 matFromQ = quatToMat(plane1Quat);
        renderPlane_geometry(plane1, matFromQ, glm::vec3(0.0f, 0.6f, 0.7f), glm::vec3(0.0f, 0.8f, 1.0f));

        // get euler angles from quaternion
        glm::vec3 eulerAngles = quatToEulerAngles(plane1Quat);
...
#define IMGUI_WINDOW_HEIGHT 205
#define IMGUI_WINDOW_MARGIN 5
    ...
    ImGui::Begin("Configurations");
        ...
        bool isTargetFacingDirModified = false;
        if (ImGui::SliderFloat("X", &targetFacingDir.x, -1.0f, 1.0f, "%.2f"))
            isTargetFacingDirModified = true;
        if (ImGui::SliderFloat("Y", &targetFacingDir.y, -1.0f, 1.0f, "%.2f"))
            isTargetFacingDirModified = true;
        if (ImGui::SliderFloat("Z", &targetFacingDir.z, -1.0f, 1.0f, "%.2f"))
            isTargetFacingDirModified = true;

        if (isTargetFacingDirModified)
        {
            // update corresponding quat
            targetFacingQuat = quatRotateBetweenVectors(glm::vec3(0.0f, 0.0f, 1.0f), targetFacingDir);
        }
        ...
        if (ImGui::IsItemHovered())
        {
            ...
            ImGui::TextUnformatted("Blue plane transformed by quaterion + slerp.\nRed-ish plane transformed by matrix converted from the same quaternion used by Blue plane.");
