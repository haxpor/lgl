 ...
 *
 *  Demonstrate rotation around the arbitrary axis constructed via the method as shown in
 *  F.Dunn 3D math book at physical page 110 onwards.
 *
 *  Actually glm library already had this implemented in its glm::rotate() and has the same implementation
 *  as of F.Dunn's version.
 *
 *  We still need to specify the order of rotation but it doesn't experience the same gimbal lock
 *  problem as this approach's rotation matrix is independent from cardinal axes compared to
 *  the approach used by Euler Angles as its rotational matrix is based on cardinal axes.
 *
 *  It's more expensive computative approach compared to a single directional vector but preserve
 *  all degrees of freedom in rotation included z-axis. Note again, stick to rotation order you defined.
 */
#include "imgui.h"
...
void render()
{
    ...
    glBindVertexArray(sharedVAO);
        ...
        computePlaneCorners(plane1, orientation, plane1CornersVertices);
