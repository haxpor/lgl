 ...
 *
 * Demonstrate algorithm used to generate quadratic bezier curve.
 * It provides UI to configure a control point which influences a final result of bezier curve with
 * additional to customization of starting and ending point in 3d space.
 *
 * Bezier Curve is a base of analogy to further study Quaternion's squad.
 */
#include "imgui.h"
...
void renderGizmo();
void renderGUI();
void traceQuadraticBezierCurve(const glm::vec3& p0, const glm::vec3& p1, const glm::vec3& p2, glm::vec3 outCurve[], int numSegment);
...
void render()
{
    ...
    glBindVertexArray(sharedVAO);
        ...
        // bezier curve
        traceQuadraticBezierCurve(p0, p1, p2, curve, CURVE_NUM_SEGMENT);
...
#define IMGUI_WINDOW_HEIGHT 330
#define IMGUI_WINDOW_MARGIN 5
    ...
    ImGui::Begin("Configurations");
        ...
        ImGui::SliderFloat("X##2", &p0.x, -1.0f, 1.0f, "%.2f");
        ImGui::SliderFloat("Y##2", &p0.y, -1.0f, 1.0f, "%.2f");
        ImGui::SliderFloat("Z##2", &p0.z, -1.0f, 1.0f, "%.2f");

        // p2
        ImGui::TextColored(ImVec4(0.0f, 0.0f, 1.0f, 1.0f), "p2 (ending point)");
        ImGui::SliderFloat("X##3", &p2.x, -1.0f, 1.0f, "%.2f");
        ImGui::SliderFloat("Y##3", &p2.y, -1.0f, 1.0f, "%.2f");
        ImGui::SliderFloat("Z##3", &p2.z, -1.0f, 1.0f, "%.2f");
        ...
        if (ImGui::IsItemHovered())
        {
            ...
            ImGui::TextUnformatted("Configure a control point, starting or ending point to influence how final bezier might look like.");
...
/// trace to produce quadratic bezier curve
/// p0 is the starting position, p2 is the ending position
/// p1 is control point
/// numSegment is number of stepping to trace, the more value is, more smooth of the curve line is.
void traceQuadraticBezierCurve(const glm::vec3& p0, const glm::vec3& p1, const glm::vec3& p2, glm::vec3 outCurve[], int numSegment)
{
    // from the closed form of V(t) = (1-t)^2*P0 + 2*(1-t)*P1 + (t^2)*P2
    for (int i=0; i<numSegment; ++i)
    {
        float t = (i+1.0f)/numSegment;
        outCurve[i] = (1-t)*(1-t)*p0 + 2*(1-t)*t*p1 + t*t*p2;
    }
}
